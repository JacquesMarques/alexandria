...
RSpec.describe 'Publishers', type: :request do
  ...
  describe 'GET /api/publishers' do
    # Before any test, let's create our 3 publishers
    before { publishers }
    ...
    describe 'field picking' do
      context 'with the fields parameter' do
        ...
        it 'gets publishers with only the id, name and keys' do
    ...
    describe 'filtering' do
      context 'with valid filtering param "q[name_cont]=Microscope"' do
        it 'receives "Ruby under a microscope" back' do
          get('/api/publishers?q[name_cont]=Dev Media')
          expect(json_body['data'].first['id']).to eq dev_media.id
          expect(json_body['data'].size).to eq 1
        end
      end

      context 'with invalid filtering param "q[fname_cont]=Microscope"' do
        before { get('/api/publishers?q[fname_cont]=Ruby') }
        ...
        it 'receives "q[fname_cont]=Ruby" as an invalid param' do
          expect(json_body['error']['invalid_params']).to eq 'q[fname_cont]=Ruby'
        end
      end
  ...
  describe 'GET /api/publishers/:id' do
    ...
    context 'with existing resource' do
      ...
      it 'receives the "oreilly" book as JSON' do
  ...
  end # describe 'GET /api/publishers/:id'

  describe 'POST /api/publishers' do
    let(:publisher) { create(:dev_media) }
    ...
    context 'with invalid parameters' do
      let(:params) { attributes_for(:michael_hartl, name: '') }
  ...
  end # describe 'PATCH /api/publishers/:id' end
  ...
  end # describe 'DELETE /api/publishers/:id' end
